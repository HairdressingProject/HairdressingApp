 ...
 * @param {string} token - GUID token generated by the server, present in the query parameters of the requested URL
 * @param {string} URL
 */
async function setNewPassword(userNameOrEmail, password, token, URL) {
    const options = createRequestHeader('PUT', {
        UserNameOrEmail: userNameOrEmail,
        UserPassword: password
    });

    const response = await fetch(`${URL}/api/users/${token}/change_password`, options);
    return handleResponse(response);
}
